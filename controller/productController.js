...
const getAllProduct = asyncHandler(async (req, res) => {
  try {
    ...
    // pagination
    //(localhost:5000/api/product?page=1&limit=3)
...
const deleteImages = asyncHandler(async (req, res) => {
  ...
  console.log(id);
  try {
    const product = await Product.findOne({ 'images.public_id': id }, { images: 1 }, );
    console.log(product.images);
    const updatedProduct = await Product.findOneAndUpdate(
      { _id: product._id },
      // { images: images.filter(image => image.public_id !== id) },
      { images: product.images.filter(image =>image.public_id != id) },
      { new: true }
    );
